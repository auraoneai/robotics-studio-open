...
Telemetry never includes raw robot media, Hugging Face tokens, AuraOne intake tokens, local file paths outside a redacted dataset root, reviewer notes, or free-text anomaly notes. Event types and privacy constraints are defined by `packages/platform-contracts/src/telemetry.ts`.
