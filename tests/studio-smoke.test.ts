...
assert.equal(state.tabs[0]?.indexedEpisodes, 3);
assert.equal(state.sensorQA.status, "fail");
assert.ok(state.clusters.some((cluster) => cluster.label === "gripper_slip:glass"));
assert.ok(firstValueMs < 60000, `first-run sample tour took ${firstValueMs}ms`);
assert.ok(firstRunHtml.includes("SO-101 kitchen counter"));
assert.equal(filterEpisodes(sampleDatasetTab.episodes, "success=false failure_modes:*").length, 2);
