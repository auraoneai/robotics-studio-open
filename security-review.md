...
- Platform controls are inherited from the vendored AuraOne Open Studio source packages.
- Tauri CSP is inherited from `packages/platform-contracts` and limits renderer network egress to the shared update, intake, telemetry, and crash-reporting endpoints. Hugging Face export runs through the sidecar and does not expand renderer CSP.
